...
- выбор чекпоинта модели перед извлечением;
- переименование чекпоинтов в интерфейсе без изменения имени папки на диске;
- ручная разметка документов в отдельной вкладке `Annotation & training`;
- создание наборов аннотаций, ввод ключа и значения, выделение bbox левой кнопкой мыши прямо на изображении;
- отображение нормализованных координат bbox в диапазоне `0..999`;
- экспорт ручной разметки в JSONL-формат DeepSeek-OCR;
- запуск подготовки обучения по размеченному набору с настройкой количества шагов;
...
- `scripts/train_annotations_lora.py` - подготовка запуска обучения по ручной разметке.
- `checkpoint-1/` - LoRA-адаптер.
- `deepseek_ocr/` - базовая модель DeepSeek-OCR.
- `data/documents.sqlite3` - база данных.
- `data/checkpoint_names.json` - отображаемые имена чекпоинтов.
- `data/annotation_exports/` - экспортированные наборы ручной разметки.
...
## Ручная разметка и обучение

Для ручной подготовки данных используется вкладка `Annotation & training`.

1. Открыть вкладку `Annotation & training`.
2. Создать набор разметки кнопкой `New dataset` или оставить поле `Active dataset` пустым: при первом сохранении набор будет создан автоматически.
3. Выбрать группу документов и конкретный документ.
4. Ввести `Key` и правильное `Value`.
5. Левой кнопкой мыши выделить область значения на изображении. После выделения ниже появятся координаты bbox в формате `[x1, y1, x2, y2]`.
6. Нажать `Save annotation`.
7. При необходимости указать количество шагов в поле `Training steps`.
8. Нажать `Train model`, чтобы экспортировать набор разметки и запустить подготовку обучения.

Разметка экспортируется в папку:

```text
data/annotation_exports/
```

Экспорт сохраняется в JSONL-формате DeepSeek-OCR:

```text
<|ref|>text<|/ref|><|det|>[[x1,y1,x2,y2]]<|/det|>значение
```

Кнопка `Train model` запускает `scripts/train_annotations_lora.py`. По умолчанию скрипт подготавливает датасет и manifest, но не начинает тяжелое GPU-дообучение, если не задана переменная `KVP_TRAIN_COMMAND`.

Пример переменной для подключения собственной команды обучения:

```powershell
$env:KVP_TRAIN_COMMAND="python train.py --dataset {dataset} --output-dir {output_dir} --max-steps {steps}"
```

Плейсхолдеры:

- `{dataset}` - путь к экспортированному JSONL с ручной разметкой;
- `{output_dir}` - папка для нового чекпоинта;
- `{steps}` - количество шагов из поля `Training steps`.

Новые чекпоинты предполагается сохранять в:

```text
trained_checkpoints/
```

После появления чекпоинта он будет доступен в выпадающем списке на основной вкладке извлечения. Кнопка `Rename` меняет только отображаемое имя чекпоинта в интерфейсе и не переименовывает папку на диске.
